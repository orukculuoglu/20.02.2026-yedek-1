/**
 * Fleet Intelligence & Risk Panel
 * Moved from: Yönetici Konsolu > Yönetici Analiz Paneli
 * New home: Filo Kiralama > Filo Zekâ ve Risk Paneli
 * 
 * Displays fleet-wide analytics:
 * - Average Part Life (Ortalama Parça Ömrü)
 * - Failure Frequency (Arıza Frekansı)
 * - Fleet Risk Score (Filo Risk Skoru)
 * - Brand-based Risk Distribution (Marka Bazlı Risk Dağılımı)
 * - Life vs Failure Correlation (Ömür vs. Arıza Korelasyonu)
 * - Strategic Signal Table (Stratejik Sinyal Tablosu)
 */

import React, { useEffect, useState } from 'react';
import { 
    Activity, AlertTriangle, Shield, 
    BarChart3, Layers, Zap, EyeOff,
    Briefcase, FileText, Building2, BrainCircuit
} from 'lucide-react';
import { 
...
interface FleetIntelligenceRiskPanelProps {
    fleetSize?: number;
}

export const FleetIntelligenceRiskPanel: React.FC<FleetIntelligenceRiskPanelProps> = ({ fleetSize: propFleetSize }) => {
    const [metrics, setMetrics] = useState<BrandAggregate[]>([]);
    const [fleetSize, setFleetSize] = useState(propFleetSize || 0);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error("Fleet Intelligence aggregation failed", e);
                setInsufficientSignal(true);
                setLoading(false);
            }
        };
        ...
        return (
            <div className="flex items-center justify-center h-screen bg-slate-50">
                <div className="text-center">
                    ...
                    <p className="text-slate-500 font-medium">Filo Zekâ Verileri Hesaplanıyor...</p>
                </div>
            </div>
        );
    }
    ...
    if (insufficientSignal) {
        return (
            <div className="p-12 flex flex-col items-center justify-center h-full bg-slate-50 text-center">
            ...
            <div className="flex justify-between items-start mb-10">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                        ...
                        Filo Zekâ ve Risk Paneli
                    </h1>
                    <p className="text-slate-500 font-medium mt-2 max-w-2xl">
                        Parça ömrü, arıza frekansı, marka/model riskleri ve stratejik sinyalleri analiz edin.
                    </p>
                </div>
