...
import { PartMasterSnapshot, Supplier, SupplierPartEdge, Part } from '../types/partMaster';

export default function B2BNetwork() {
  const [snapshot, setSnapshot] = useState<PartMasterSnapshot | null>(null);
  ...
  useEffect(() => {
    console.log('[B2BNetwork.tsx] Loading PartMasterSnapshot...');
    
    getPartMasterSnapshot(tenantId).then(data => {
      console.log('[B2BNetwork.tsx] Snapshot loaded:', {
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-12 text-slate-400">
        ...
        <p className="font-bold text-sm">Part Master Ağı Yükleniyor...</p>
      </div>
    );
  }
  ...
  const filteredParts = snapshot.parts.filter(p => {
    const isConnected = connectedPartIds.includes(p.partId);
    const matchesSearch = 
    ...
    <div className="h-full flex flex-col animate-in fade-in duration-500 bg-slate-50/50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 pb-0">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">{snapshot.suppliers.length}</h3>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">{snapshot.parts.length}</h3>
          </div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <h3 className="text-2xl font-bold text-slate-800">{snapshot.edges.length}</h3>
          </div>
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  ...
                  <div className="flex items-center gap-1 text-[10px] font-bold text-amber-600">
                    <Star size={10} fill="currentColor" /> {(sup.reliabilityScore || 85)}%
                  </div>
                </div>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 text-[10px] text-slate-400 uppercase font-black tracking-widest sticky top-0 z-10 border-b border-slate-100">
                      <tr>
                        ...
                        <th className="px-6 py-3">Yapı</th>
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {filteredParts.map(part => {
                        const edge = connectedEdges.find(e => e.partId === part.partId);
                        return (
                          <tr key={part.partId} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-6 py-4">
                              ...
                              <div className="text-[9px] font-mono text-slate-400 mt-0.5">{part.sku} • {part.brand || 'N/A'}</div>
                            </td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-1.5 text-[10px] font-bold text-indigo-600">
                                <Truck size={12} /> {edge?.leadDays || 7} Gün
                              </div>
                            </td>
                            <td className="px-6 py-4">
                              <div className="flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-sm" />
                                <span className="text-xs font-bold text-slate-700">{part.packSize || 1} {part.unit}</span>
                              </div>
                            </td>
                            <td className="px-6 py-4 text-right">
                              <span className="text-xs font-black text-slate-900">{(edge?.unitPrice || 0).toLocaleString()} ₺</span>
                            </td>
                            <td className="px-6 py-4 text-right">
      ...
      <div className="mx-6 mb-6 p-4 bg-indigo-900 text-indigo-100 rounded-2xl flex items-start gap-4 shadow-xl shadow-indigo-900/20 relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <h5 className="font-bold text-sm mb-1">SafeCore Part Master Supply Intelligence v2.0</h5>
          <p className="text-[11px] text-indigo-300 leading-relaxed max-w-4xl">
            Tedarikçi ağındaki fiyat ve stok sinyalleri PartMaster canonical modeli üzerinden yönetilmektedir.
            Tüm tedarikçi, parça ve kenar veriler tek bir kaynaktan (PM Snapshot) beslenir. Sipariş onayı için ERP ile senkronizasyon gereklidir.
          </p>
        </div>
      </div>
    </div>
