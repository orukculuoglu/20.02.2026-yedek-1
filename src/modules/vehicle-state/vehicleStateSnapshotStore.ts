...
export type VehicleStateSnapshot = {
  ...
  vehicleIntelligenceRecommendations?: Array<{
    ...
    priorityScore?: number;          // Phase 10.4: Priority ranking
    group?: 'maintenance' | 'risk' | 'insurance' | 'data-quality';  // Phase 10.5: Category
  }>;

  // Phase 11.1+: Vehicle Intelligence Actions (generated from recommendations)
  // Structured operational tasks derived from recommendations
  // Generated by actionEngine based on vehicleIntelligenceRecommendations
  // Stored async by actionOrchestrator, NOT by reducer
  vehicleIntelligenceActions?: Array<{
    key: string;                     // Unique action identifier
    title: string;                   // User-facing action title (Turkish)
    summary: string;                 // Brief description
    actionType: string;              // Type of action (e.g., SCHEDULE_MAINTENANCE)
